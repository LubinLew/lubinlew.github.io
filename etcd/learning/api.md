...
本文档旨在概述etcd3 API的中央设计。 它绝不是全部内容，而是旨在专注于理解etcd所需的基本思想，而不会分散不太常见的API调用。 所有etcd3 API均在[gRPC服务] [grpc-service]中定义，该API对etcd服务器可理解的远程过程调用（RPC）进行了分类。 所有etcd RPC的完整列表在[gRPC API列表] [grpc-api]的markdown中记录。
