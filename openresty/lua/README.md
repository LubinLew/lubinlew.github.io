...
LuaJIT 对标准 Lua 5.1 是向上兼容的。它支持所有[标准 Lua 库函数](https://www.lua.org/manual/5.1/manual.html#5)和所有 [Lua/C API 函数](https://www.lua.org/manual/5.1/manual.html#3).

LuaJIT 同样是完全 ABI 级别兼容 Lua 5.1 的 链接和动态加载。This means you can compile a C module against the standard Lua headers and load the same shared library from either Lua or LuaJIT.

LuaJIT 扩展了 标准 Lua VM(虚拟机)，增加了新的函数和新的模块。当然与新功能相比，LuaJIT 的主要特点是高性能， 它优化了Lua虚拟机，拥有更快的解释器 和 并加入了JIT编译器。
...
This module is a LuaJIT built-in — you don't need to download or install Lua BitOp. The Lua BitOp site has full documentation for all [Lua BitOp API functions](https://bitop.luajit.org/api.html).
...
LuaJIT adds some [extra functions to the Lua/C API](https://luajit.org/ext_c_api.html).
...
An extra argument has been added to string.dump(). If set to true, 'stripped' bytecode without debug information is generated. This speeds up later bytecode loading and reduces memory usage. See also the [-b command line option](https://luajit.org/running.html#opt_b).
