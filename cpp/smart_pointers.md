...
• `unique_ptr<T>` 对象类似于指向T类型的指针，是唯一的，这表示不能有多个 `unique_ptr<T>` 对象保存相同的地址。换句话说，从不会有两个或吏多个 `unique_ptr<T>` 对象同时指向相同的地址。`unique_ptr<T>` 对象独占地拥有指向的内容。编译器通过不允许复制 `unique_ptr<T>` 來强制这种唯-性。虽然不能复制`unique_ptr<T>`,但是使用 std::move() 函数,可以把一个 `unique_ptr<T>` 对象存储的地址移动到另一个`unique_ptr<T>` 对象中。执行该操作后，最初的智能指针就再次变为空的。

• `shared_ptr<T>` 对象类似于指向T类型的指针， 但与  `unique_ptr<T>`  不同，可以有任意多个 `shared_ptr<T>` 对象包含或共享相同的地址。因此，`shared_ptr<T>` 对象允许共享自由存储区中对象的所有权。在任何给定时刻，运行时知道包含给定地址的 `shared_ptr<T>` 对象的个数，称为引用计数。每次创建包含给定白由存储区地址的新 `shared_ptr<T>` 对象时，包含该地址的 `shared_ptr<T>` 引用计数会递增；包含该地址的 `shared_ptr<T>` 对象被释放或指向另一个地址吋，该引用计数会递减。没有包含给定地址的`shared_ptr<T>` 对象时，引用计数为0,位于该地址的对象的内存会自动释放。指向相同地址的所有 `shared_ptr<T>` 对象都可以访问引用计数。

• `weak_ptr<T>` 被链接到 `shared_ptr<T>` 上，包含相同的地址。创建 ·weak_ptr<T>· 不会递增所链接的 `shared_ptr<T>` 对象的引用计数，所以不能防止它指向的对象被释放。引用它的最后一个 `shared_pt<T>`对象被释放或重置为指向另一个地址吋，即使链接的weak_ ptK "D >对象仍存在，其内存也会释放。即使发生这种情况 `weak_ptr<T>` 也不会包含悬挂指针，至少不会包含程序员会无意访问的指针。原因在于，程序员不能直接访
问 `weak_ptr<T>` 封装的地址。编译器会强制首先从weak -p trcP^y建一个sh ared_p tr<T>对象来引用相冋的地址。如果we ak _p tK l^ l用的内存地址依然有效，强制创建sh ar ed _p tK T> 首先会确保引用计数再次递增，并且指针玎再次被安全使用。但是，如果内存己经释放，创建的sharedjJtKl^将包含uullptr。
weak_plK T>对象的一种用途是避免创建share djrt Kh 对象的引用循环。在概念上，引用循环是指对象x内的
sh ar ed _p tr <T >指向其他某个对象y, 而y包禽的sh ar ed _p tt <T >又指回X。在这种情况下，这两个对象都不能释放。
实际上，情形会复杂得多。weak_ptr<T>对象用于避免引用循环的问题。弱指针的另一种用途是实现对象缓存。
读者可能己经感受到，弱指针仅用于比较高级的川例。因为它们只会被偶尔使用，这里不再继续讨论它们。但
是，另外两种智能指针类型非常常用，所以接下来深入介绍它们
